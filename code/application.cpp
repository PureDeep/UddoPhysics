...
int GetTimeMicroseconds()
{
    ...
    const unsigned __int64 timeMicro = static_cast<unsigned __int64>(static_cast<double>(tick - gStartTicks) /
        ticks_per_micro);
...
void Application::InitializeGLFW()
{
    ...
    m_glfwWindow = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "UddoPhysics", nullptr, nullptr);
...
bool Application::InitializeVulkan()
{
    ...
    //
    if (VK_SUCCESS != glfwCreateWindowSurface(m_deviceContext.m_vkInstance, m_glfwWindow, nullptr,
                                              &m_deviceContext.m_vkSurface))
    {
...
void Application::MainLoop()
{
    ...
    while (!glfwWindowShouldClose(m_glfwWindow))
    {
        ...
        if (runPhysics)
        {
            ...
            dt_us = static_cast<float>(endTime) - static_cast<float>(startTime);
            if (dt_us > maxTime)
            {
                maxTime = dt_us;
            }

            avgTime = (avgTime * static_cast<float>(numSamples) + dt_us) / static_cast<float>(numSamples + 1);
            numSamples++;
